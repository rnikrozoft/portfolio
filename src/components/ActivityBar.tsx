...
export default function ActivityBar({
  ...
  useEffect(() => {
    dispatch(
      sectionSlice.actions.setSections({ sections: sections[pathname] })
    );
      ...
      <div className="max-w-fit text-gray-500 flex flex-col justify-between h-full">
        <div className="cursor-pointer">
          <Tooltip
            ...
            handleMouseClick={() => {
              dispatch(
                expandableSlice.actions.toggleMenu({ menu: Menu.EXPLORER })
              );
              ...
              setTimeout(() => {
                dispatch(
                  explorerSlice.actions.toggleMenu({
          ...
          {barItems.map((item, index) => (
            <Tooltip
              ...
              handleMouseClick={() => {
                dispatch(
                  expandableSlice.actions.toggleMenu({ menu: item.menu })
                );
              }}
            />
          ))}
